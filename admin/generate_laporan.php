...
if (isset($_GET['bulan'])) {
    $bulan = $_GET['bulan'];
    $filter_sql = "WHERE DATE_FORMAT(tanggal_pinjam, '%Y-%m') = '$bulan'";
} else {
    $filter_sql = "";
}

// Query untuk mendapatkan data peminjaman
$sql = "SELECT peminjaman_id, buku.judul, user.username, tanggal_pinjam, tanggal_kembali, status_peminjam
        FROM peminjaman
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row_data = mysqli_fetch_assoc($result)) {
        ...
        $sheet->setCellValue('E' . $row, $row_data['tanggal_kembali']);
